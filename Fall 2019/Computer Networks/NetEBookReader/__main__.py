...
from Ui_TxtReader import txtreader
...
class Reader(QMainWindow, Ui_MainWindow):
    def __init__(self, parent=None):
        ...
        self.tcp = tcp_client()
        # 下载图书元数据database（文件名、图书信息）
        print('[Reader]从服务器端请求图书元数据……')
        self.tcp.download("PDF.db", "PDF_1.db")
        maintain_db()
        print('[Reader]请求及更新元数据成功')
        # 设置应用图标
    ...
    def initUi(self):
        ...
        self.addbar.triggered.connect(self.open)  # （网络版中不予使用）
        # 信息
        # 连接
        self._init_bookset()
    ...
    def set_icon(self, fname):
        ...
        self.tcp.download(fname)  # 网络版中，使用从服务器下载的jpg文件渲染封面
        # 打开 PDF
        # doc = fitz.open(fname)
        # 加载封面
        # page = doc.loadPage(0)
    ...
    def generate_menu(self, pos):
        ...
        if (row_num < self.crow.x) or (row_num == self.crow.x and col_num <= self.crow.y):
            ...
            item2 = menu.addAction('删除图书')   # 网络版中，该选项的含义是删除图书的本地缓存
            ...
            if action == item1:
                ...
                if book.fname[-3:] == 'txt':
                    fmt = '文件格式：txt'
                elif os.path.exists(book.fname):
                    book.get_meta_data(book.fname)
                    info = book.info
                    fmt = f'文件格式：pdf\n\n' \
                          f'文件名：{info.path}\n\n' \
                          f'格式：{info.format}\n\n' \
    ...
    # 删除图书
    def delete_book(self, fname):  # row, col):
        """
        # 获取图书在列表中的位置
        index = row * 8 + col
    ...
    def read_book(self, book):
        ...
        if fname[-3:] == 'pdf':
            if not os.path.exists(fname):
                print('[Reader]从服务器端请求图书……')
                self.tcp.download(fname)
                print('[Reader]请求图书成功')
            book.get_meta_data(fname)
            doc = fitz.open(fname)
